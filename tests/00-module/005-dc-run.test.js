...
describe('docker-compose run mongo', function () {
    this.timeout(30000);

    var file = __dirname + '/../docker-compose.yaml';
    it('Execute command run', done => {

        module.dockerComposeRun(file, 'mongo', 'echo OK').then(out => {
            expect(out.indexOf('OK')).to.not.equal(-1);
            done();
        }, done);
